...
> 版本：v1.0 ｜ 更新：2026-09-02 ｜ 状态：阶段 1 已完成（可运行闭环）
...
没有文档台账（无法增量更新），也没有标签体系。参照豆包方案，把知识库**整体重构**为
...
### 阶段 2：标签体系 + 切片缓存 —— ⬜ 待做
- [ ] `tagger.py`：LLM 零样本打标签（读 tag_schema.json 词表约束）
- [ ] `chunk_cache.py`：切片 jsonl 缓存，避免重复分块
- [ ] 入库流程接入 tagger + chunk_cache
- 验收：丢一篇新 md，ingest 后 doc_index 的 tags 非空

### 阶段 2.5：删除清理 —— ⬜ 待做
- [ ] ingest 里对 removed 文档：从向量库删对应片段（按 source）
- 验收：删文件 → ingest → query 不再命中

### 阶段 3：工具调用 + 入口 —— ⬜ 待做
- [ ] kb 版 TOOLS（search_knowledge / execute_python / web_search）+ function calling 循环
- [ ] tagger 也可作为工具暴露给 Agent
- 验收：`"用 python 算 2 的 10 次方"` 触发 execute_python

### 阶段 4：多 Agent 协作（LangGraph）—— ⬜ 待做
- [ ] Planner → Retriever → Answerer → Reviewer 图（参考旧项目 graph.py）
- [ ] Overview 节点 / 条件边打回 / 来源三态 / 阈值 0.6
- 验收：复杂问题触发重写循环，弱相关不硬答

### 阶段 5：Web 界面 + API（FastAPI）—— ⬜ 待做
- [ ] FastAPI：/api/ingest /api/chat /api/stats /api/docs
- [ ] 前端（DeepSeek 风格深色页，参考旧项目 static/）
- 验收：浏览器提问有回答 + 来源
